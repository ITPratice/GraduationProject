...
namespace VehicleTracker.ViewModel
{
    public class VehicleTypeVM
    {
        ...
        public static async Task<String> DeleteVehicleTypeAsync(HttpClient client, String id, String name)
        {
            client.BaseAddress = new Uri(Constants.API_BASE_URL);
            var res = await client.DeleteAsync($"api/vehicletype?id={id}&name={name}");
            res.EnsureSuccessStatusCode();
            var strResult = await res.Content.ReadAsStringAsync();
            return strResult;
        }
